...
export class FullAggridComponent implements OnInit, OnDestroy {
      ...
      children: [
        {
          ...
          filter: 'agNumberColumnFilter',
          width: 100,
          // cellRenderer → custom HTML per cell; avoid heavy renderers on 10k+ rows
          cellRenderer: (p: ICellRendererParams) =>
            p.value > 0
              ? `<span style="font-weight:700;color:#b8860b">${p.value}</span>`
  ...
  // Fires when user edits an editable cell
  onCellValueChanged(e: CellValueChangedEvent): void {
    console.log(`[CellEdit] field=${e.colDef.field} old=${e.oldValue} new=${e.newValue}`);
    // Persist to API here: this.myService.update(e.data)
  }

  onSortChanged(_e: SortChangedEvent): void {
    // Retrieve state: this.gridApi.getColumnState()
  }

  onFilterChanged(_e: FilterChangedEvent): void {
    const model = this.gridApi?.getFilterModel();
    console.log('[FilterModel]', model);
    // Restore with: this.gridApi.setFilterModel(model)
  }

  onPaginationChanged(_e: PaginationChangedEvent): void {
    // this.gridApi.paginationGetCurrentPage()  → 0-based page index
    // this.gridApi.paginationGetTotalPages()   → total pages
  }

  onGridSizeChanged(_e: GridSizeChangedEvent): void {
  ...
  addRow(row: OlympicRow): void { this.gridApi?.applyTransaction({ add: [row], addIndex: 0 }); }
  updateRow(row: OlympicRow): void { this.gridApi?.applyTransaction({ update: [row] }); }
  removeRow(row: OlympicRow): void { this.gridApi?.applyTransaction({ remove: [row] }); }
